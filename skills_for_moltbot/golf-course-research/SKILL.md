...
## Post-Research: Proposal and Ingestion

After completing the research and generating the final JSON, **immediately** handle ingestion using the course-proposal-manager skill:

### Step 1: Import Proposal Manager
...
### Step 2: Check Auto-Ingest Mode

```python
# After research completes and you have the final course_json
course_json = {...}  # Your complete JSON structure

# Check if auto-ingest is enabled
result = auto_ingest_if_enabled(course_json)
if result:
    # Auto-ingest mode: already ingested
    return {
        "status": "ingested",
        "course_id": result["courseId"],
        "snapshot_id": result["snapshotId"],
        "message": f"Ingested ✅ Course ID: `{result['courseId']}`\nSnapshot: `{result['snapshotId']}`"
    }
```

### Step 3: Create Proposal (Confirm-Then-Ingest Mode)

If auto-ingest is disabled, create a proposal:
...
```python
    ...
    proposal_id = create_proposal(
        ...
        run_id=operation_id  # Current operation ID if available
    )
    ...
    return {
        ...
        "message": message,
        "telegram_buttons": [
            [{"text": "✅ Ingest", "callback_data": f"proposal:ingest:{proposal_id}"}],
            [{"text": "📄 View JSON", "callback_data": f"proposal:view_json:{proposal_id}"}],
            [{"text": "⏭️ Skip", "callback_data": f"proposal:skip:{proposal_id}"}]
        ]
    }
```
...
When you receive a proposal result, send it via Telegram with inline buttons:
...
if result["status"] == "proposal_created":
    ...
    telegram_message = {
        "text": result["message"],
        "reply_markup": {
            "inline_keyboard": result["telegram_buttons"]
        },
        "parse_mode": "Markdown"
    }
    # Send via your Telegram integration
    send_telegram_message(chat_id, telegram_message)
```

**Important Notes:**
- Always check `AUTO_INGEST` environment variable first
- Use the exact callback_data format: `proposal:{action}:{proposal_id}`
- Store the original message_id if you need to edit it later
- The proposal manager handles all storage and API calls
- See `/root/clawd/skills/course-proposal-manager/TELEGRAM_INTEGRATION.md` for callback handling
